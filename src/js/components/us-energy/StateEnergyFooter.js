	...
	getLinkHtml(stateIds) {
        ...
        const links = stateIds.map(state => {
            ...
            let html = (
                <a 
                    href={href}
                    key={`energy-footer-${state}`}
                    className="state-energy-footer-link col-6 col-sm-4 col-md-3 col-lg-2"
                    target={target}>{getFullStateName(state)}
                </a>
            );
            return html;
        })
	...
	render() {
		...
		return (
            <div className="state-energy-footer-text-container" >
                ...
                <div className="state-energy-footer row">
                    {stateIdLinks}
                </div>
            </div>
		);
