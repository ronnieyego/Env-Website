...
export default class EnergySourceMap extends React.Component {
    ...
    getEnergyFilterOptions(energySourceArray) {
        ...
        let list = sources.map(source => { return <li><input type="radio" id={source} defaultChecked={source === this.state.currentSource} name="source" value={source} key={source} onClick={this.filterMap.bind(this)} /> {getSourceDisplayname(source)}</li> });
        return list;
    }

    getUtilityFilterOptions(utilitySourceArray) {
        let sources = utilitySourceArray.sort();
        let list = sources.map(source => { return <li> <input type="radio" id={source} defaultChecked={source === this.state.currentUtility} name="utility" value={source} key={source} onClick={this.filterMap.bind(this)} /> {getProducerDisplayname(source)}</li> });
        return list;
    }
