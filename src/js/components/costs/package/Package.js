    ...
    getDisplayTextFromResults(results) {
        ...
        if(results.fromOverseas) {
            ...
            if(results.sameState) {
                text = `Unfortunately this tool isn't meant to calculate CO2 within the same state.  Its likely that the carrier will transport this package exclusively via truck.  I'm estimating that it'll travel ~${fromStateToHouse} miles from your house to its destination.  Please adjust the results based on the actual distance.`
            } else if(results.local) {
                text = `This package starts at your house.  You drive it to the post office or UPS and it magically arrives in ${getFullStateName(results.destinationState)}.  Behind the scenes, the mailing company trucks it to their distribution center and then start routing it.  Since the destination is somewhat close (under 200 miles) to your house, it was probably faster and cheaper to ship the package by truck instead of plane or rail.  It travelled about ${results.usDistance.toLocaleString()} miles to a distribution center in ${getFullStateName(results.destinationState)}.  Finally, the package was loaded into a different truck and travelled ~${fromStateToHouse} miles to its destination.`;
            } else if (results.rush) {
                text = `This package starts at your house.  You drive it to the post office or UPS and it magically arrives in ${getFullStateName(results.destinationState)}.  Behind the scenes, the mailing company trucks it to their distribution center and then start routing it.  Since you needed this package quickly, it had to be rushed over. This means it travelled ${results.airDistance.toLocaleString()} miles by air and landed in the closest hub in ${getFullStateName(results.destinationState)}. The package was then loaded into a truck and travelled ~${fromStateToHouse} miles to its destination.`;
            } 
            else {
                text = `This package starts at your house.  You drive it to the post office or UPS and it magically arrives in ${getFullStateName(results.destinationState)}.  Behind the scenes, the mailing company trucks it to their distribution center and then start routing it. Usually its cheapest and easiest to transport packages via train.  From there it travelled about ${results.usDistance.toLocaleString()} miles to a major hub in ${getFullStateName(results.destinationState)}.  Finally, the package was loaded into a truck and travelled ~${fromStateToHouse} miles to its destination.`;
            }
        }
